...
inter['faixa_etaria'] = inter.faixa_etaria.replace(dict(faixaetaria.values)).copy()
inter['traducao_libras'] = inter.traducao_libras.replace(dict(libras.values)).copy()
...
logo = html.Img(
    ...
    style={'height':'50px', 'width':'80px', 'margin-left':'60px', 'margin-bottom':'20px', 'margin-top':'10px'}
) # width = largura

drop1 = dcc.Dropdown( 
    options=[{'label':i, 'value':i} for i in sorted(faixaetaria.faixa_etaria.unique())],
    id='drop_idade',
...
drop2 = dcc.Dropdown( 
    options=[{'label':i, 'value':i} for i in libras.trad_libras.unique()[-1:0:-1]],
    id='assistencia',
    placeholder='Clique para selecionar',
    #value='Sim'
)
...
drop3 = dcc.Dropdown( 
    options=[{'label':i, 'value':i} for i in estados.estado.values], # COLOCAR ESTADOS
    id='estados',
    placeholder='Clique para selecionar',
    #multi=True
    #value='BAHIA'
...
bt1 = dbc.Button('Resetar filtros', id='btn1', color="secondary", size="sm", style={'margin-left':'0px'})
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([ # COLUNA 1
            ...
            html.Div([
                html.Label(children=['Filtro de faixa etária:'], id='l1'), # CAIXA 1
                dbc.Col([drop1], style={'margin-bottom':dist_labdrops}), # DROPDOWN 1

                html.Label(children=['Interprete de Libras:'], id='l2'), # CAIXA 2
                dbc.Col([drop2], style={'margin-bottom':dist_labdrops}), # DROPDOWN 2

                html.Label(children=['Filtro por Estado:'], id='l3'), # CAIXA 3
                dbc.Col([drop3], style={'margin-bottom':dist_labdrops}), # DROPDOWN 3

                html.Label(children=['Tipos de visualização:'], id='l4', style={'margin-top':'15px'}), # CAIXA 4
        ...
        dbc.Col([ # COLUNA 2
            ...
            dbc.Row([slicer], style={'height':'10vh', 'margin-top':'50px'}), # C2 L1
            dbc.Row([mapa], style={'margin-top':'20px', 'margin-right':'1px', 'height':'75vh'}) # C2 L2
...
def ver_resultado(drop_idade, drop_assistencia, drop_estado, btn1, check):
    ...
    temp = temp[['id_museu', 'eventId']].groupby('id_museu', as_index=None).count()
    ...
    temp['estado_completo'] = temp['estado_completo'].replace(dict(estados[['id_estados', 'estado']].values)).copy()
    ...
    if temp.empty:
        ...
        if check == 'Localização dos museus':
            centro = dict(lat=temp.latitude.mean()+2.3, lon=temp.longitude.mean())
            zoom = 3 if drop_estado is None else 4
        elif check == 'Eventos por museu':
            centro = dict(lat=temp.latitude.mean()+4, lon=temp.longitude.mean())
    ...
    fig_1.update_layout(
        mapbox=dict(center=go.layout.mapbox.Center(centro)), # outra maneira -> lat=-16.6, lon=-50.6)),
        template='plotly_dark',  # plotly_dark
        paper_bgcolor=('rgba(0, 0, 0, 0)'),
        margin={'l': 2, 'r': 0, 't': 10, 'b': 2}, # ou margin=go.layout.Margin(l=2, r=0, t=10, b=2)
